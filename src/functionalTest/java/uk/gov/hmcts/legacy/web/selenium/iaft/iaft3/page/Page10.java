...
public class Page10 extends AbstractIAFTPage implements IaftPage {
	...
	@Override
	public void execute() {
		logger.info("Executing IAFT3 Step 10");
		...
		if (fieldLength == 18) {
			...
			val = fields[17];
			if (val.equals(NavigateEnum.FINISH.getValue())) {
				clickNextButton();
			} else if (val.equals(NavigateEnum.PREVIOUS.getValue())) {
				clickPreviousButton();
			} else {
				logger.info("Val:" + val + " not understood expected one of:" + NavigateEnum.FINISH.getValue()

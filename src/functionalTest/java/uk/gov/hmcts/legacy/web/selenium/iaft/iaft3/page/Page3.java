...
public class Page3 extends AbstractIAFTPage implements IaftPage {
	...
	public static final String SERVICE_DECSION_METHOD_FAX_XPATH = "//*[@id=\"s2d_fax\"]";
	...
	public static final String SERVICE_DEC_DAY_NAME = "s2g_service_day";

	public static final String SERVICE_DEC_MONTH_NAME = "s2g_service_month";

	public static final String SERVICE_DEC_YEAR_NAME = "s2g_service_year";

	@Override
	public void execute() {
		logger.info("Executing IAFT3 Step 3");
		...
		if (fieldLength == 15) {
			...
			val = fields[5];
			if (val != null) {
				findSelectByNameAndSelectByValue(SERVICE_DEC_DAY_NAME, val);
			} else {
				for (String day : DAYS) {
					findSelectByNameAndSelectByValue(SERVICE_DEC_DAY_NAME, day);
				}
			}

			val = fields[6];
			if (val != null) {
				findSelectByNameAndSelectByValue(SERVICE_DEC_MONTH_NAME, val);
			} else {
				for (String month : MONTHS) {
					findSelectByNameAndSelectByValue(SERVICE_DEC_MONTH_NAME, month);
				}
			}

			val = fields[7];
			if (val != null) {
				findSelectByNameAndSelectByValue(SERVICE_DEC_YEAR_NAME, val);
			} else {
				for (String year : TWOMINUS_YEARS) {
					findSelectByNameAndSelectByValue(SERVICE_DEC_YEAR_NAME, year);
				}
			}
			...
			if (val != null) {
				...
				for (String month : MONTHS) {
					findSelectByNameAndSelectByValue(REFUSAL_DAY_NAME, month);
				}
			}
			...
			if (val != null) {
				...
				for (String month : MONTHS) {
					findSelectByNameAndSelectByValue(SERVICE_DAY_NAME, month);
				}
			}
			...
			logger.info("Page 3 not initialised with correct number of fields. Expected 15 was " + fieldLength);

...
public class PDFUtilityIAFT6 extends Action {
	/**
	 * log4j logging solution
	 */
	static Logger logger = Logger.getLogger(PDFUtilityIAFT6.class);
	...
	public static ByteArrayOutputStream GenerateIAFT6PDF(String path, boolean paymentStatus, IAFT6Bean frm)
			throws IOException, DocumentException {

		logger.info("PDFUtilityIAFT6.GenerateIAFT6PDF");

		try {
			String template = "";

			if (paymentStatus == true)
				template = "templates/IAFT6-online.pdf";
			else
				template = "templates/IAFT6-online.pdf";
...
*/
				...
				logger.error("PDFUtilityIAFT6.GenerateIAFT6PDF - ", ex);
			}
			...
			try {
				...
				if (frm.getS2f_day() != 0)
					form.setField("s2g",frm.getS2f_day() + "/" + frm.getS2f_month() + "/"+ frm.getS2f_year());
			...
			try {
				...
				if (frm.getS3d_1() != null) {
					if(frm.getFormtitle().equals("Human Rights"))
					{
						form.setField("s3d_heading", "Human Rights Decision/EEA Decision");
						form.setField("s3d_question", "If you consider your human rights are affected by the decision against which you are appealing please explain which article of the Human Rights Convention you are relying upon and give reasons to support your claim. Alternatively, if you consider that your EEA right of free movement is affected by the decision against which you are appealing, please explain why");
					}
					else
					{
						form.setField("s3d_heading", "Human Rights Decision/EEA Decision");

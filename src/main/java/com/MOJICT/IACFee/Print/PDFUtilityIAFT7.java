package com.MOJICT.IACFee.Print;
...
public class PDFUtilityIAFT7 extends Action {
	/**
	 * log4j logging solution
	 */
	static Logger logger = Logger.getLogger(PDFUtilityIAFT7.class);
	...
	public static ByteArrayOutputStream GenerateIAFT7PDF(String path, boolean paymentStatus, IAFT7Bean frm)
			throws IOException, DocumentException {

		logger.info("PDFUtilityIAFT7.GenerateIAFT7PDF");

		try {
			String template = "";

			if (paymentStatus == true)
				template = "templates/IAFT7-online.pdf";
			else
				template = "templates/IAFT7-online.pdf";
			...
			// Page 1
			try {
				form.setField("submission_txt", "Appellant Copy");
...
*/				
				...
				logger.error("PDFUtilityIAFT7.GenerateIAFT7PDF - ", ex);
			}
			...
			try {
				...
				else if(frm.getFormtitle().equals("Human Rights"))
					form.setField("tod_rdo", "2");
				else if(frm.getFormtitle().equals("Protection Revoke"))
					form.setField("tod_rdo", "3");
				else if(frm.getFormtitle().equals("EEA"))
					form.setField("tod_rdo", "4");
				else if(frm.getFormtitle().equals("Citizenship"))
					form.setField("tod_rdo", "5");
				// Appeal Type
				if (frm.getAppeal_type().equals("Oral"))
			...
			try {
				...
				if (frm.getS2hoan() != null)
					form.setField("s2hoan", frm.getS2hoan());
				if (frm.getS2j_service_day() != 0)
					form.setField(
							"s2j",
							frm.getS2j_service_day() + "/" + frm.getS2j_service_month() + "/"
				...
				if (frm.getS2f_service_day() != 0)
					form.setField(
							"s2h",
							frm.getS2f_service_day() + "/" + frm.getS2f_service_month() + "/"
									+ frm.getS2f_service_year());
				if (frm.getS2i_service_day() != 0)
					form.setField("s2i",frm.getS2i_service_day() + "/" + frm.getS2i_service_month() + "/"+ frm.getS2i_service_year());
                if(frm.getS2k()!=null)
                {
                	if (frm.getS2k().equals("Yes"))
						form.setField("s2g_rdo", "1");
                	else
                		form.setField("s2g_rdo", "2");
                }
			} catch (Exception ex) {
				logger.error("PDFUtilityIAFT7.GenerateIAFT7PDF - ", ex);
			}

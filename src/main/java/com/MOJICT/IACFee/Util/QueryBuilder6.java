...
/**
 * The Class QueryBuilder6.
 */
public class QueryBuilder6 extends GenericQueryBuilder implements
		QueryBuilder6Properties {

	/** log4j logging solution. */
	static Logger logger = Logger.getLogger(QueryBuilder6.class);
	...
	public static IAFT6Bean retrieve(IAFT6Bean frm, String action,
                                     DataSource datasource, String security_token, HttpServletRequest req) {
		IAFT6Bean bean = new IAFT6Bean();
		...
		try {
			...
			String thissql = sql_retrieve6_1;
			String thissql2 = sql_retrieve6_2;
			String thissql3 = sql_retrieve6_33;
			String thissql4 = sql_retrieve6_4;
			String thissql5 = sql_retrieve6_5;
			String thissql6 = sql_retrieve6_6;
			String thissql7 = sql_retrieve6_7;
			String thissql8 = sql_retrieve6_8;
			String thissql9 = sql_retrieve6_9;
			...
			if (rs.next()) {
				...
				bean.setS2f(rs.getDate(55));
				// dateconverter.convertdate(bean.getS2e());
				if (rs.getDate(55) != null) {
					bean.setS2f_day(rs.getDate(55).getDate());
					bean.setS2f_month(rs.getDate(55).getMonth() + 1);
					bean.setS2f_year(1900 + rs.getDate(55).getYear());
				}
			...
			logger.error("QueryBuilder6.retrieve", e);
			return bean;
		} finally {
			if (rs != null) {
				try {
					...
					logger.error("QueryBuilder6.retrieve", sqle);
				}
				rs = null;
			}
			if (conn != null) {
	 ...
	 * @return the iAF t2 bean
	 */
	public static IAFT6Bean IAFT6_1(IAFT6Bean frm, String action,
			DataSource datasource, String security_token, HttpServletRequest req) {
		IAFT6Bean bean = new IAFT6Bean();
		Connection conn = null;
		PreparedStatement stmt = null;
		...
		if (action.equals("retrieve")) {
			...
			return QueryBuilder6.retrieve(frm, action, datasource,
					security_token, req);
		}
			...
			try {
                ...
                {
                	IAFT6Bean iaf=(IAFT6Bean)QueryBuilder6.retrieve(frm, action, datasource, security_token, req);
                	if (frm.getQb_lsc().equals("No")) {
				...
				if (save.equals("update")) {
					...
					if(req.getSession().getAttribute("frmHeader")!=null)
					{
						if(req.getSession().getAttribute("frmHeader").toString().equals("Human Rights"))
						{
						stmt1.setString(5, "Human Rights");
						}
						else
						{
							stmt1.setString(5, "EEA");
						}
					}
					else
					{
					...
					return QueryBuilder6.retrieve(frm, action, datasource, req
					...
					if(req.getSession().getAttribute("frmHeader")!=null)
					{
						if(req.getSession().getAttribute("frmHeader").toString().equals("Human Rights"))
						{
						stmt1.setString(7, "Human Rights");
						}
						else
						{
							stmt1.setString(7, "EEA");
						}
					}
					else
					{
				...
				logger.error("QueryBuilder6.IAFT6_1", e);
				return bean;
			} finally {
				try {
					closeResultSet(rs);
					...
					closeConnection(conn);
					logger.debug("\t\tMethod: IAFT6_2 Status:Save End");
				} catch (SQLException sqle) {
					logger.error("QueryBuilder6.IAFT6_1", sqle);
				}
			}
	 ...
	 * @return the iAF t2 bean
	 */
	public static IAFT6Bean IAFT6_2(IAFT6Bean frm, String action,
			DataSource datasource, String security_token, HttpServletRequest req) {
		...
		if (action == "retrieve") {
			try {
				...
				logger.error("QueryBuilder6.IAFT6_2", e);
				return bean;
			}
			finally {
				try {
					...
					logger.error("QueryBuilder6.IAFT6_2", sqle);
				}
			}
		}
	 ...
	 * @return the iAF t2 bean
	 */
	public static IAFT6Bean IAFT6_3(IAFT6Bean frm, String action,
			DataSource datasource, String security_token, HttpServletRequest req) {
		...
		ResultSet rs = null;
		IAFT6Bean bean = new IAFT6Bean();
		IAFT6Bean bean_amount = new IAFT6Bean();
		if (action == "retrieve") {
			try {
				DBConnection dbconnection = new DBConnection();
				conn=dbconnection.getConnection();

				String thissql = sql_retrieve6_3;
				String thissql11 = sql_update6_2urn;
				stmt = conn.prepareStatement(thissql);
				stmt1 = conn.prepareStatement(thissql11);
				stmt.setString(1, security_token);
				rs = stmt.executeQuery();
				...
				logger.error("QueryBuilder6.IAFT6_3", e);
				return bean;
			}
			finally {
				try {
					...
					logger.error("QueryBuilder6.IAFT6_3", sqle);
				}
			}
		}
		...
		if (action == "save") {
			...
			try {
				...
				frm.setS2i(dateconverter.convertdate(frm.getS2i_day(), frm.getS2i_month(),
						frm.getS2i_year()));
				frm.setS2f(dateconverter.convertdate(frm.getS2f_day(), frm
						.getS2f_month(), frm.getS2f_year()));

				stmt.setDate(7,frm.getS2f());

				stmt.setDate(8,frm.getS2i());
				stmt.setString(9, security_token);
				bean_amount = QueryBuilder6.retrieve(frm, action, datasource,
						security_token, req);
				// stmt1.setString(5, "IAFT6");
	 ...
	 * @return the iAF t2 bean
	 */
	public static IAFT6Bean IAFT6_4(IAFT6Bean frm, String action,
			DataSource datasource, String security_token, HttpServletRequest req) {
		...
		if (action == "retrieve") {
			try {
				...
				logger.error("QueryBuilder6.IAFT6_4", e);
				return bean;
			}
			finally {
				try {
					...
					logger.error("QueryBuilder6.IAFT6_4", sqle);
				}
			}
		}
	 ...
	 * @return the iAF t2 bean
	 */
	public static IAFT6Bean IAFT6_5(IAFT6Bean frm, String action,
			DataSource datasource, String security_token, HttpServletRequest req) {
		...
		if (action == "retrieve") {
			try {
				...
				logger.error("QueryBuilder6.IAFT6_5", e);
				return bean;
			}
			finally {
				try {
					...
					closeConnection(conn);
					logger.debug("\t\tMethod: IAFT6_5 Status:Save End");
				} catch (SQLException sqle) {
					logger.error("QueryBuilder6.IAFT6_5", sqle);
				}
			}
		}
		...
		if (action == "save") {
			...
			try {
				DBConnection dbconnection = new DBConnection();conn=dbconnection.getConnection();
				String thissql1 = sql_save6_5_1;
				stmt1 = conn.prepareStatement(thissql1);
				stmt1.setString(1, frm.getS3d_1());
				stmt1.setString(2, security_token);
				stmt1.executeUpdate();
				return QueryBuilder6.IAFT6_6(frm, action, datasource,
						security_token, req);
			} catch (Exception e) {
				logger.error("QueryBuilder6.IAFT6_5", e);
				return bean;
	 ...
	 * @return the iAF t2 bean
	 */
	public static IAFT6Bean IAFT6_6(IAFT6Bean frm, String action,
			DataSource datasource, String security_token, HttpServletRequest req) {
		...
		if (action == "retrieve") {
			try {
				...
				logger.error("QueryBuilder6.IAFT6_6", e);
				return bean;
			}finally {
				try {
					closeResultSet(rs);
					...
					closeConnection(conn);
					logger.debug("\t\tMethod: IAFT6_6 Status:Save End");
				} catch (SQLException sqle) {
					logger.error("QueryBuilder6.IAFT6_6", sqle);
				}
			}
		}
		if (action == "save") {
	 ...
	 * @return the iAF t2 bean
	 */
	public static IAFT6Bean IAFT6_7(IAFT6Bean frm, String action,
			DataSource datasource, String security_token, HttpServletRequest req) {
		...
		if (action == "retrieve") {
			try {
				...
				logger.error("QueryBuilder6.IAFT6_7", e);
			}
			finally {
				try {
					closeResultSet(rs);
					...
					logger.error("QueryBuilder6.IAFT6_7", sqle);
				}
			}
		}
	 ...
	 * @return the string
	 */
	public static String IAFT6_8(IAFT6Bean frm, String action,
			DataSource datasource, String security_token, HttpServletRequest req) {
		...
		if (action == "retrieve") {
			try {
				...
				logger.error("QueryBuilder6.IAFT6_8", e);
				return "failure";
			}finally {
				try {
					closeResultSet(rs);
					...
					logger.error("QueryBuilder6.IAFT6_8", sqle);
				}
			}
		}
	 ...
	 * @return the iAF t2 bean
	 */
	public static IAFT6Bean IAFT6_9(IAFT6Bean frm, String action,
			DataSource datasource, String security_token, HttpServletRequest req) {
		...
		if (action == "save") {
			...
			try {
				...
				logger.error("QueryBuilder6.IAFT6_9", e);
				return frm;
			} finally {
				try {
					closeResultSet(rs);
					...
					logger.error("QueryBuilder6.IAFT6_9", sqle);
				}
			}

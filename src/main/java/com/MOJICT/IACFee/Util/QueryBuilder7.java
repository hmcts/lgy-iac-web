...
/**
 * The Class QueryBuilder7.
 */
public class QueryBuilder7 extends GenericQueryBuilder implements QueryBuilder7Properties {
	
	/** log4j logging solution. */
	static Logger logger = Logger.getLogger(QueryBuilder7.class);
	 ...
	 *             the sQL exception
	 */
	private IAFT7Bean populateIAFT7Bean(IAFT7Bean bean, HttpServletRequest req, String security_token,Connection conn)
			throws SQLException {
...
//		Connection conn = null;
		...
		if (rsR.next()) {
			...
			if (rsR.getDate(55) != null) {
				theDate = rsR.getDate(55);
				bean.setS2f_service_day(getTheDay(theDate));
				bean.setS2f_service_month(getTheMonth(theDate));
				bean.setS2f_service_year(getTheYear(theDate));
			}
			...
			bean.setS2hoan(rsR.getString(99));
			
			if (rsR.getDate(100) != null) {
				theDate = rsR.getDate(100);
				bean.setS2j_service_day(getTheDay(theDate));
				bean.setS2j_service_month(getTheMonth(theDate));
				bean.setS2j_service_year(getTheYear(theDate));
			}
			bean.setS2k(rsR.getString(101));
			bean.setS3g(rsR.getString(102));
	 ...
	 */
	public static IAFT7Bean retrieve(IAFT7Bean frm, String action, DataSource datasource, String security_token,
			HttpServletRequest req) {
		Connection conn = null;
		IAFT7Bean bean = new IAFT7Bean();
		QueryBuilder7 qBuilder = new QueryBuilder7();
		try {
			
			DBConnection dbconnection = new DBConnection();
			conn=dbconnection.getConnection();
 
			bean = qBuilder.populateIAFT7Bean(bean, req, security_token,conn);
		} catch (Exception e) {
			logger.error("QueryBuilder7.retrieve", e);
			return bean;
		}finally {
			try {
				
				QueryBuilder7.closeConnection(conn);
				logger.debug("\t\tMethod: IAFT7_2 Status:Save End");
			} catch (SQLException sqle) {
				logger.error("QueryBuilder7.retrieve", sqle);
			}
		}
		return bean;
	}
	 ...
	 * @return the iAF t1 bean
	 */
	public static IAFT7Bean IAFT7_1(IAFT7Bean frm, String action, DataSource datasource, String security_token,
			HttpServletRequest req) {
		Connection conn = null;
		IAFT7Bean bean = new IAFT7Bean();
		...
		if (action.equals("retrieve")) {
			return QueryBuilder7.retrieve(frm, action, datasource, security_token, req);
		}
			...
			try {
				...
				if (!save.equals("update")) {
					...
					if (req.getSession().getAttribute("frmHeader").toString().equals("Protection")) {
						bean.setSubmissionURN("51" + "88" + SecurityToken.generateSubmissionURN(submission_urn));
						logger.debug("Assylum");
					} 
					if (req.getSession().getAttribute("frmHeader").toString().equals("Protection Revoke")) {
						bean.setSubmissionURN("53" + "88" + SecurityToken.generateSubmissionURN(submission_urn));
						logger.debug("Assylum");
					}
					if (req.getSession().getAttribute("frmHeader").toString().equals("Human Rights")) {
						bean.setSubmissionURN("52" + "88" + SecurityToken.generateSubmissionURN(submission_urn));
						logger.debug("Assylum");
					}
					if (req.getSession().getAttribute("frmHeader").toString().equals("EEA")) {
						bean.setSubmissionURN("54" + "88" + SecurityToken.generateSubmissionURN(submission_urn));
						logger.debug("Assylum");
					}
					if (req.getSession().getAttribute("frmHeader").toString().equals("Citizenship")) {
						bean.setSubmissionURN("55" + "88" + SecurityToken.generateSubmissionURN(submission_urn));
						logger.debug("Assylum");
					}
					}
				...
				if (req.getSession().getAttribute("aggrtoken") != null) {
					...
					if (req.getSession().getAttribute("frmHeader").toString().equals("Protection")) {
						bean.setSubmissionURN("51" + "77" + SecurityToken.generateSubmissionURN(submission_urn));
						logger.debug("Assylum");
					} 
					if (req.getSession().getAttribute("frmHeader").toString().equals("Protection Revoke")) {
						bean.setSubmissionURN("53" + "77" + SecurityToken.generateSubmissionURN(submission_urn));
						logger.debug("Assylum");
					}
					if (req.getSession().getAttribute("frmHeader").toString().equals("Human Rights")) {
						bean.setSubmissionURN("52" + "77" + SecurityToken.generateSubmissionURN(submission_urn));
						logger.debug("Assylum");
					}
					if (req.getSession().getAttribute("frmHeader").toString().equals("EEA")) {
						bean.setSubmissionURN("54" + "77" + SecurityToken.generateSubmissionURN(submission_urn));
						logger.debug("Assylum");
					}
					if (req.getSession().getAttribute("frmHeader").toString().equals("Citizenship")) {
						bean.setSubmissionURN("55" + "77" + SecurityToken.generateSubmissionURN(submission_urn));
						logger.debug("Assylum");
					}
					bean.setPaymentURN(bean.getSubmissionURN());
				}
				if (!frm.getQb_lsc().equals("No") || feepayable != null) {
					if (req.getSession().getAttribute("frmHeader").toString().equals("Protection")) {
						bean.setSubmissionURN("51" + "99" + SecurityToken.generateSubmissionURN(submission_urn));
						logger.debug("Assylum");
					} 
					if (req.getSession().getAttribute("frmHeader").toString().equals("Protection Revoke")) {
						bean.setSubmissionURN("53" + "99" + SecurityToken.generateSubmissionURN(submission_urn));
						logger.debug("Assylum");
					}
					if (req.getSession().getAttribute("frmHeader").toString().equals("Human Rights")) {
						bean.setSubmissionURN("52" + "99" + SecurityToken.generateSubmissionURN(submission_urn));
						logger.debug("Assylum");
					}
					if (req.getSession().getAttribute("frmHeader").toString().equals("EEA")) {
						bean.setSubmissionURN("54" + "99" + SecurityToken.generateSubmissionURN(submission_urn));
						logger.debug("Assylum");
					}
					if (req.getSession().getAttribute("frmHeader").toString().equals("Citizenship")) {
						bean.setSubmissionURN("55" + "99" + SecurityToken.generateSubmissionURN(submission_urn));
						logger.debug("Assylum");
					}
				...
				{
					IAFT7Bean iaf=(IAFT7Bean)QueryBuilder7.retrieve(frm, action, datasource, security_token, req);
					if (frm.getQb_lsc().equals("No")) {
						...
						{
						if (req.getSession().getAttribute("frmHeader").toString().equals("Protection")) {
							bean.setSubmissionURN("51" + "88" + iaf.getSubmissionURN().substring(4));
							logger.debug("Assylum");
						} 
						if (req.getSession().getAttribute("frmHeader").toString().equals("Protection Revoke")) {
							bean.setSubmissionURN("53" + "88" + iaf.getSubmissionURN().substring(4));
							logger.debug("Assylum");
						}
						if (req.getSession().getAttribute("frmHeader").toString().equals("Human Rights")) {
							bean.setSubmissionURN("52" + "88" + iaf.getSubmissionURN().substring(4));
							logger.debug("Assylum");
						}
						if (req.getSession().getAttribute("frmHeader").toString().equals("EEA")) {
							bean.setSubmissionURN("54" + "88" + iaf.getSubmissionURN().substring(4));
							logger.debug("Assylum");
						}
						if (req.getSession().getAttribute("frmHeader").toString().equals("Citizenship")) {
							bean.setSubmissionURN("55" + "88" + iaf.getSubmissionURN().substring(4));
							logger.debug("Assylum");
						}
						}
						bean.setPaymentURN(bean.getSubmissionURN());
					...
					if (req.getSession().getAttribute("aggrtoken") != null) {
						...
						if (req.getSession().getAttribute("frmHeader").toString().equals("Protection")) {
							bean.setSubmissionURN("51" + "77" + iaf.getSubmissionURN().substring(4));
							logger.debug("Assylum");
						} 
						if (req.getSession().getAttribute("frmHeader").toString().equals("Protection Revoke")) {
							bean.setSubmissionURN("53" + "77" + iaf.getSubmissionURN().substring(4));
							logger.debug("Assylum");
						}
						if (req.getSession().getAttribute("frmHeader").toString().equals("Human Rights")) {
							bean.setSubmissionURN("52" + "77" + iaf.getSubmissionURN().substring(4));
							logger.debug("Assylum");
						}
						if (req.getSession().getAttribute("frmHeader").toString().equals("EEA")) {
							bean.setSubmissionURN("54" + "77" + iaf.getSubmissionURN().substring(4));
							logger.debug("Assylum");
						}
						if (req.getSession().getAttribute("frmHeader").toString().equals("Citizenship")) {
							bean.setSubmissionURN("55" + "77" + iaf.getSubmissionURN().substring(4));
							logger.debug("Assylum");
						}
						bean.setPaymentURN(bean.getSubmissionURN());
					}
					if (!frm.getQb_lsc().equals("No") || feepayable != null) {
						if (req.getSession().getAttribute("frmHeader").toString().equals("Protection")) {
							bean.setSubmissionURN("51" + "99" + iaf.getSubmissionURN().substring(4));
							logger.debug("Assylum");
						} 
						if (req.getSession().getAttribute("frmHeader").toString().equals("Protection Revoke")) {
							bean.setSubmissionURN("53" + "99" + iaf.getSubmissionURN().substring(4));
							logger.debug("Assylum");
						}
						if (req.getSession().getAttribute("frmHeader").toString().equals("Human Rights")) {
							bean.setSubmissionURN("52" + "99" + iaf.getSubmissionURN().substring(4));
							logger.debug("Assylum");
						}
						if (req.getSession().getAttribute("frmHeader").toString().equals("EEA")) {
							bean.setSubmissionURN("54" + "99" + iaf.getSubmissionURN().substring(4));
							logger.debug("Assylum");
						}
						if (req.getSession().getAttribute("frmHeader").toString().equals("Citizenship")) {
							bean.setSubmissionURN("55" + "99" + iaf.getSubmissionURN().substring(4));
							logger.debug("Assylum");
						}
						// bean.setSubmissionURN("33"+"99"+SecurityToken.generateSubmissionURN(submission_urn));
						bean.setPaymentURN(bean.getSubmissionURN());
				...
				if (save.equals("update")) {
					...
					// bean = QueryBuilder7.retrieve(frm, action, datasource,
					// security_token, req);
					IAFT7Bean iaf=(IAFT7Bean)QueryBuilder7.retrieve(frm, action, datasource, security_token, req);
					...
					stmt1.execute();
					logger.debug("\t\tMethod: IAFT7_1 Status:Update End");
					return QueryBuilder7.retrieve(frm, action, datasource, req.getSession().getAttribute("token1")
							.toString(), req);
				} else {
				...
				logger.error("QueryBuilder7.IAFT7_1", e);
				return bean;
			}finally{
				try{
				QueryBuilder7.closeConnection(conn);
				}catch (Exception r){
					logger.error("QueryBuilder7.IAFT7_1", r);
				}
			}
		}
		logger.debug("\t\tMethod: IAFT7_1 Status:Save End");
		return bean;
	}
	 ...
	 * @return the iAF t1 bean
	 */
	public static IAFT7Bean IAFT7_2(IAFT7Bean frm, String action, DataSource datasource, String security_token,
			HttpServletRequest req) {
		Connection conn = null;
		PreparedStatement stmt = null;
		ResultSet rs = null;
		...
		if (action == "retrieve") {
			try {
				...
				logger.error("QueryBuilder7.IAFT7_2", e);
				return bean;
			} finally {
				try {
					QueryBuilder7.closeResultSet(rs);
					QueryBuilder7.closePreparedStatement(stmt);
					QueryBuilder7.closeConnection(conn);
				} catch (SQLException sqle) {
					logger.error("QueryBuilder7.IAFT7_2", sqle);
				}
			}
		}
	 ...
	 * @return the iAF t1 bean
	 */
	public static IAFT7Bean IAFT7_3(IAFT7Bean frm, String action, DataSource datasource, String security_token,
			HttpServletRequest req) {

		logger.debug("\t\tMethod: IAFT7_3 Status: Start");
		Connection conn = null;
		PreparedStatement stmt = null;
		PreparedStatement stmt1 = null;
		ResultSet rs = null;
			...
			IAFT7Bean bean = new IAFT7Bean();
			IAFT7Bean bean_amount = new IAFT7Bean();
			...
			if (action == "retrieve") {
				try {
					...
					logger.error("QueryBuilder7.IAFT7_3", e);
					return bean;
				}finally {
					try {
						QueryBuilder7.closeResultSet(rs);
						...
						logger.error("QueryBuilder7.IAFT7_3", sqle);
					}
				}
			}
			...
			if (action == "save") {
				try {
					...
					frm.setS2f(dateconverter.convertdate(frm.getS2f_service_day(), frm.getS2f_service_month(),
							frm.getS2f_service_year()));
					stmt.setDate(
							6,
					...
					stmt.setDate(
							7,
							dateconverter.convertdate(frm.getS2f_service_day(), frm.getS2f_service_month(),
									frm.getS2f_service_year()));

					stmt.setDate(
							8,
							frm.getS2i());
					stmt.setString(9, frm.getS2hoan());
					frm.setS2j(dateconverter.convertdate(frm.getS2j_service_day(), frm.getS2j_service_month(),
							frm.getS2j_service_year()));
					stmt.setDate(10,frm.getS2j());
					stmt.setString(11, frm.getS2k());
					
					stmt.setString(12, security_token);

					stmt.executeUpdate();
					bean_amount = QueryBuilder7.retrieve(frm, action, datasource, security_token, req);
					closeResultSet(rs);
					closePreparedStatement(stmt);
					...
					try {
						QueryBuilder7.closeResultSet(rs);
						QueryBuilder7.closePreparedStatement(stmt);
						QueryBuilder7.closePreparedStatement(stmt1);
						QueryBuilder7.closeConnection(conn);
						logger.debug("\t\tMethod: IAFT7_3 Status: End");
					} catch (SQLException sqle) {
						logger.error("QueryBuilder7.IAFT7_3", sqle);
					}
				}
	 ...
	 * @return the iAF t1 bean
	 */
	public static IAFT7Bean IAFT7_4(IAFT7Bean frm, String action, DataSource datasource, String security_token,
			HttpServletRequest req) {

		logger.debug("\t\tMethod: IAFT7_4 Status: Start");
		Connection conn = null;
		PreparedStatement stmt = null;
		ResultSet rs = null;
		IAFT7Bean bean = new IAFT7Bean();
			...
			if (action == "retrieve") {
				try {
					...
					logger.error("QueryBuilder7.IAFT7_4", e);
					return bean;
				}finally {
					try {
						QueryBuilder7.closeResultSet(rs);
						...
						logger.error("QueryBuilder7.IAFT7_4", sqle);
					}
				}
			}
	 ...
	 * @return the iAF t1 bean
	 */
	public static IAFT7Bean IAFT7_5(IAFT7Bean frm, String action, DataSource datasource, String security_token,
			HttpServletRequest req) {

		logger.debug("\t\tMethod: IAFT7_5 Status: Start");
		IAFT7Bean bean = new IAFT7Bean();
		PreparedStatement stmt = null;
		ResultSet rs = null;
		Connection conn = null;
		if (action == "retrieve") {
			try {
				...
				logger.error("QueryBuilder7.IAFT7_5", e);
				return bean;
			} finally {
				try {
					QueryBuilder7.closeResultSet(rs);
					QueryBuilder7.closePreparedStatement(stmt);
					QueryBuilder7.closeConnection(conn);
					logger.debug("\t\tMethod: IAFT7_5 Status: End");
				} catch (SQLException sqle) {
					logger.error("QueryBuilder7.IAFT7_5", sqle);
				}
			}
		}
		...
		if (action == "save") {
			...
			try {
				...
				try {
					QueryBuilder7.closeResultSet(rs);
					QueryBuilder7.closePreparedStatement(stmt1);
					QueryBuilder7.closePreparedStatement(stmt2);
					QueryBuilder7.closePreparedStatement(stmt3);
					QueryBuilder7.closeConnection(conn);
					logger.debug("\t\tMethod: IAFT7_6 Status: End");
				} catch (SQLException sqle) {
					logger.error("QueryBuilder7.IAFT7_5", sqle);
				}
			}
	 ...
	 * @return the iAF t1 bean
	 */
	public static IAFT7Bean IAFT7_6(IAFT7Bean frm, String action, DataSource datasource, String security_token,
			HttpServletRequest req) {

		logger.debug("\t\tMethod: IAFT7_6 Status: Start");
		Connection conn = null;
		IAFT7Bean bean = new IAFT7Bean();
		ResultSet rs = null;
		PreparedStatement stmt = null;
		if (action == "retrieve") {
			try {
				...
				logger.error("QueryBuilder7.IAFT7_6", e);
				return bean;
			} finally {
				try {
					QueryBuilder7.closeResultSet(rs);
					...
					logger.error("QueryBuilder7.IAFT7_6", sqle);
				}
			}
		}
		...
		if (action == "save") {
			...
			try {
				...
				try {
					QueryBuilder7.closePreparedStatement(stmt4);
					QueryBuilder7.closePreparedStatement(stmt5);
					QueryBuilder7.closePreparedStatement(stmt6);
					QueryBuilder7.closePreparedStatement(stmt7);
					QueryBuilder7.closeConnection(conn);
					logger.debug("\t\tMethod: IAFT7_6 Status: End");
				} catch (SQLException sqle) {
					logger.error("QueryBuilder7.IAFT7_6", sqle);
	 ...
	 * @return the iAF t1 bean
	 */
	public static IAFT7Bean IAFT7_7(IAFT7Bean frm, String action, DataSource datasource, String security_token,
			HttpServletRequest req) {
		Connection conn = null;
		logger.debug("\t\tMethod: IAFT7_7 Status: Start");
		...
		if (action == "retrieve") {
			...
			try {
				...
				logger.error("QueryBuilder7.IAFT7_7", e);
				return bean;
			} finally {
				try {
					QueryBuilder7.closeResultSet(rs);
					QueryBuilder7.closePreparedStatement(stmt);
					QueryBuilder7.closeConnection(conn);
					logger.debug("\t\tMethod: IAFT7_7 Status: End");
				} catch (SQLException sqle) {
					logger.error("QueryBuilder7.IAFT7_7", sqle);
				}
			}
		}
		...
		if (action == "save") {
			...
			try {
				...
				try {
					QueryBuilder7.closePreparedStatement(stmt8);
					QueryBuilder7.closePreparedStatement(stmt9);
					QueryBuilder7.closeConnection(conn);
					logger.debug("\t\tMethod: IAFT7_7 Status: End");
				} catch (SQLException sqle) {
					logger.error("QueryBuilder7.IAFT7_7", sqle);
	 ...
	 * @return the iAF t1 bean
	 */
	public static IAFT7Bean IAFT7_8(IAFT7Bean frm, String action, DataSource datasource, String security_token,
			HttpServletRequest req) {
		Connection conn = null;
		logger.debug("\t\tMethod: IAFT7_8 Status: Start");
		PreparedStatement stmt = null;
		ResultSet rs = null;
		try {
			IAFT7Bean bean = new IAFT7Bean();
			if (action == "retrieve") {
				try {
					...
					logger.error("QueryBuilder7.IAFT7_8", e);
					return bean;
				}
			}
			...
			try {
				QueryBuilder7.closeResultSet(rs);
				QueryBuilder7.closePreparedStatement(stmt);
				logger.debug("\t\tMethod: IAFT7_8 Status: End");
				QueryBuilder7.closeConnection(conn);
			} catch (SQLException sqle) {
				logger.error("QueryBuilder7.IAFT7_8", sqle);
			}
		}
	}
	 ...
	 * @return the iAF t1 bean
	 */
	public static IAFT7Bean IAFT7_9(IAFT7Bean frm, String action, DataSource datasource, String security_token,
			HttpServletRequest req) {
		Connection conn = null;
		logger.debug("\t\tMethod: IAFT7_9 Status: Start");
		PreparedStatement stmt = null;
		ResultSet rs = null;
		try {
			IAFT7Bean bean = new IAFT7Bean();
			if (action == "retrieve") {
				try {
					...
					logger.error("QueryBuilder7.IAFT7_9", e);
					return bean;
				}finally {
					try {
						QueryBuilder7.closeResultSet(rs);
						...
						logger.error("QueryBuilder7.IAFT7_9", sqle);
					}
				}
			}
			...
			if (action == "save") {
				...
				try {
					...
					stmt.setString(4, security_token);
					stmt.executeUpdate();
					logger.debug("\t\tMethod: IAFT7_9 Status:Save End");
					return QueryBuilder7.retrieve(frm, action, datasource, security_token, req);
				} catch (Exception e) {
					logger.error("QueryBuilder7.IAFT7_9", e);
					return bean;
				...
				logger.debug("\t\tMethod: IAFT7_9 Status:Retrieve End");
				return bean;
			}
		} finally {
			try {
	 ...
	 * @return the string
	 */
	public static String IAFT7_10(IAFT7Bean frm, String action, DataSource datasource, String security_token) {
		Connection conn = null;
		logger.debug("\t\tMethod: IAFT7_10 Status: Start");
		PreparedStatement stmt = null;
		ResultSet rs = null;
		try {
			if (action == "retrieve") {
				try {
					...
					logger.error("QueryBuilder7.IAFT7_10", e);
					return "failure";
				}
			}
			...
			if (action == "save") {
				...
				logger.debug("\t\tMethod: IAFT7_10 Status: Save end");
				return "success";
			} else {
				logger.debug("\t\tMethod: IAFT7_10 Status: retrieve end");
				return "success";
			}
		} finally {
			try {
				...
				logger.error("QueryBuilder7.IAFT7_10", sqle);
			}
		}
	}
	...
	public static ArrayList<AggregatedSubmissionBean> getAggregatedsubmission(DataSource datasource, String urn) {
		...
		try {
			...
			logger.error("QueryBuilder7.getAggregatedsubmission", e);
			return asBeansArrayList;
		} finally {
			try {
				QueryBuilder7.closeResultSet(rs);
				...
				logger.error("QueryBuilder7.getAggregatedsubmission", sqle);
			}
		}
	}
	...
	public static String gettoken(int no ,HttpServletRequest request) {
		...
		return token;
			...
			logger.error("QueryBuilder7.gettoken", e);
		return token;
			...
			try {
				...
				logger.error("QueryBuilder7.gettoken", sqle);
			}
		}
	}
	...
	public static ArrayList<AggregatedSubmissionBean> getAggregatedsubmissiontoken(DataSource datasource, String urn) {
		...
		try {
			...
			logger.error("QueryBuilder7.getAggregatedsubmissiontoken", e);
			return asBeansArrayList;
		} finally {
			try {
				QueryBuilder7.closeResultSet(rs);
				...
				logger.error("QueryBuilder7.getAggregatedsubmissiontoken", sqle);
			}
		}
	}
	...
	public static String  updateappealstatus(DataSource datasource, String token,String status) {
		...
		try {
			...
			logger.error("QueryBuilder7.updateappealstatus", e);
			return "failure";
		} finally {
			try {
				QueryBuilder7.closeResultSet(rs);
				...
				logger.error("QueryBuilder7.updateappealstatus", sqle);
			}
		}
	}
	 ...
	 * @return the iAF t1 bean
	 */
	public static IAFT7Bean IAFT_new(IAFT7Bean frm, String action, DataSource datasource, String security_token,
			HttpServletRequest req) {
		IAFT7Bean bean = new IAFT7Bean();
		...
		try {
			logger.debug("coming here in IAFT3  new lodgement");
			String thissql1 = sql_update_urn;

			DBConnection dbconnection = new DBConnection();conn=dbconnection.getConnection();

			stmt1 = conn.prepareStatement(thissql1);
			//frm = QueryBuilder7.retrieve(frm, "retrieve", datasource, security_token, req);
			if (req.getSession().getAttribute("aggrtoken") != null) {
			...
			logger.error("QueryBuilder7.IAFT_new", e);
			...
			try {
				...
				logger.error("QueryBuilder7.IAFT_new", sqle);
			}
		}
    ...
    PreparedStatement stmt1=null;
		...
		{
			logger.error("QueryBuilder7.updateurn", e);
		}
		finally {
			try {
				//QueryBuilder7.closeResultSet(rs);
				...
				logger.error("QueryBuilder7.updateurn", sqle);
			}
		}
